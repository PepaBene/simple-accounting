...
const IFRS_ACCOUNT_CLASSES: Record<string, string> = {
  '1': 'Assets',
  '2': 'Liabilities',
  '3': 'Equity',
  '4': 'Revenue',
...
export const ifrsStandard: AccountingStandard = {
  id: 'ifrs',
  ...
  classifyAccount(code: string, type: AccountType): AccountClassification {
    ...
    const className = IFRS_ACCOUNT_CLASSES[classDigit] || 'Unknown class';
  ...
  getAccountClassName(code: string): string {
    const classDigit = getAccountClassDigit(code);
    return IFRS_ACCOUNT_CLASSES[classDigit] || 'Unknown class';
  },
};
